...
Сценарий: 13.01 Создаем вид отчета с рекурсией расчета показателей
	...
	* Настраиваем структуру вида отчета
		...
		И Я в конструкторе отчета добавляю строку с именем 'Receiver'
	...
	* Вводим значение
		...
		Если '$$LanguageИнтерфейса$$ = "Ru"' Тогда 
			Тогда табличный документ 'SpreadsheetDocumentField' равен:
				| 'Errors'                                                                                                                              | '' | ''                                                                                                                     |
				| 'Обнаружена рекурсия в связях When_ расчете pokazatelja: "Receiver sum".\nCheck формулы rascheta и перезапишите правило rascheta.'   | '' | '{"S","Tree rascheta:\n	Total sum\n		Accumulation sum\n			Receiver sum\n				Source sum\n				Total sum"}'      |
				| 'Обнаружена рекурсия в связях When_ расчете pokazatelja: "Accumulation sum".\nCheck формулы rascheta и перезапишите правило rascheta.' | '' | '{"S","Tree rascheta:\n	Receiver sum\n		Source sum\n		Total sum\n			Accumulation sum\n				Receiver sum"}'    |
				| 'Обнаружена рекурсия в связях When_ расчете pokazatelja: "Total sum".\nCheck формулы rascheta и перезапишите правило rascheta.'       | '' | '{"S","Tree rascheta:\n	Accumulation sum\n		Receiver sum\n			Source sum\n			Total sum\n				Accumulation sum"}' |
		ИначеЕсли '$$LanguageИнтерфейса$$ = "En"' Тогда
			Тогда табличный документ 'SpreadsheetDocumentField' равен:
				| 'Errors'                                                                                                                                        | '' | ''                                                                                                                       |
				| 'Recursion is found in links when calculating the indicator: "Receiver sum".\nCheck calculation formulas and rewrite the calculation rule.'   | '' | '{"S","Calculation tree:\n	Total sum\n		Accumulation sum\n			Receiver sum\n				Source sum\n				Total sum"}'      |
				| 'Recursion is found in links when calculating the indicator: "Accumulation sum".\nCheck calculation formulas and rewrite the calculation rule.' | '' | '{"S","Calculation tree:\n	Receiver sum\n		Source sum\n		Total sum\n			Accumulation sum\n				Receiver sum"}'    |
				| 'Recursion is found in links when calculating the indicator: "Total sum".\nCheck calculation formulas and rewrite the calculation rule.'       | '' | '{"S","Calculation tree:\n	Accumulation sum\n		Receiver sum\n			Source sum\n			Total sum\n				Accumulation sum"}' |
...
Сценарий: 13.03 Перенастраиваем структуру вида отчета
	...
	* Приемник
		...
		И в табличном документе 'SpreadsheetFieldTemlate' я перехожу к ячейке 'R3C2'
		И в табличном документе 'SpreadsheetFieldTemlate' я делаю двойной клик на текущей ячейке
		И в поле с именем 'TextDocFieldProcedure' я ввожу текст '[VATransformationPeriod_Source_Sum]'
		И я нажимаю на кнопку с именем 'WriteAndCollapse'				
	...
	* Накопление
		...
		И Я выбираю показатель с кодом 'Accumulation_Sum'
		Тогда открылось окно "Data source (create) *"
		...
		Тогда открылось окно "Edit tree *"
		И в поле с именем 'TextDocFieldProcedure' я ввожу текст '[VATransformationPeriod_Accumulation_Sum01] + [VATransformationPeriod_Receiver_Sum]'
		И я нажимаю на кнопку с именем 'WriteAndCollapse'
	...
	* Итог
		...
		Тогда открылось окно 'VA - Transformation period_Accumulation Sum (Data source)'
		И я перехожу к закладке с именем 'FiltersPage'
		И в таблице 'TreeOfFilterParametersDB' я перехожу к строке:
			| "Filter"         | "Field"            |
			| "Report period" | "[Report period]" |
		...
		И я нажимаю на кнопку с именем 'FormWriteAndClose'
		И я жду закрытия окна 'VA - Transformation period_Accumulation Sum (Data source) *' в течение 20 секунд
		Тогда открылось окно "Data sources"
...
Сценарий: 13.04 Проверяем расчет в сводной таблице
	...
	* Вводим значения
		...
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| 'VA - Transformation period (pivot table)' | 'Sum'          | 'Sum'           | 'Sum'        | 'Sum' |
			| 'Source'                                      | '100'            | '0'               | '0'            | '100'   |
			| 'Receiver'                                      | '100'            | '0'               | '0'            | '100'   |
