...
Сценарий: 12.01 Настраиваем значения аналитик по умолчанию
	...
	* Настраиваем Номенклатуру	
		* Находим вид аналитики
			...
			Тогда открылось окно "Dimension kinds (corporate)"
			И в таблице 'List' я выбираю текущую строку
			Тогда открылось окно 'Products (Dimension kinds (corporate))'
			И В текущем окне я нажимаю кнопку командного интерфейса "Default dimension values"
		* Удаляем текущие позиции
			И пока в таблице 'List' количество строк '>' 0 Тогда
				И я выбираю пункт контекстного меню с именем 'ListContextMenuDelete' на элементе формы с именем 'List'
		...
		* Создаем значение на февраль
			...
			Если '$$ЭтоPerform$$' Тогда
				...
				И в таблице 'List' я выбираю текущую строку
			Иначе
				И из выпадающего списка с именем 'DefaultValue' я выбираю по строке '1C:Corporate performance management'
			И я нажимаю на кнопку с именем 'FormWriteAndClose'
			И я жду закрытия окна "Default dimension values (create) *" в течение 20 секунд
		...
		* Закрываем окна
			Тогда открылось окно 'Products (Dimension kinds (corporate))'
			И Я закрываю окно 'Products (Dimension kinds (corporate))'
...
Сценарий: 12.02 Создаем вид отчета и структуру отчета
	...
	* Устанавливаем формулы
		* Сумма
			...
			Тогда открылось окно 'VA - Filling empty analytics_Source sum (Data source)'
			...
			И в таблице 'MappingTable' я перехожу к строке:
				| "Destination dimension"       | "Dimension kind" | "Column name"                 | "Filling method" |
				| 'Dimension 3: Products' | 'Products'  | '[Dimension 3: Products]' | "Source field"    |
			И в таблице 'MappingTable' я выбираю текущую строку
			И в таблице 'MappingTable' из выпадающего списка с именем 'MappingTableFillingMethod' я выбираю точное значение "Fixed value"
			И в таблице 'MappingTable' я завершаю редактирование строки
			И я нажимаю на кнопку с именем 'FormWriteAndClose'
			И я жду закрытия окна 'VA - Filling empty analytics_Source sum (Data source) *' в течение 20 секунд
			Тогда открылось окно "Data sources"
	...
	И Я Для вида отчета 'VA - Filling empty analytics' в бланке для группы раскрытия с адресом 'R9C1' задаю сортировку 'Products.Product ID'
	И Я Для вида отчета 'VA - Filling empty analytics' в бланке для группы раскрытия с адресом 'R12C1' задаю сортировку 'Products.Product ID'
	И Я Для вида отчета 'VA - Filling empty analytics' в бланке для группы раскрытия с адресом 'R15C1' задаю сортировку 'Products.Product ID'
...
Сценарий: 12.03 Создаем экземпляр отчета без записи и заполнения аналитик
	...
	* Сравниваем результат
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| ''                                | 'Sum'          | 'Date' | 'Sum'           | 'Date'       | 'Sum'        | 'Date' | 'Sum' | 'Date'       |
...
Сценарий: 12.04 Создаем экземпляр отчета без записи с заполнением аналитик
	...
	* Сравниваем результат
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| '1C:Corporate performance management '      | '0'              | ''           | '200'             | '2/1/2021' | '0'            | ''           | '200'   | '2/1/2021' |
...
Сценарий: 12.05 Добавляем расчет для источника
	...
	* Добавляем формулу
		* Сумма
			...
			Тогда открылось окно 'VA - Filling empty analytics, : ParameterValueCalculation'
