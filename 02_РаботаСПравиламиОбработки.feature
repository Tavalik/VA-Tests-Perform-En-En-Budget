...
Сценарий: 02.01 Создание группы отчетов 'VA - Rules rascheta (group)'

	И Я создаю группу видов отчетов с именем 'VA - Rules rascheta (group)' и родителем 'VA - Report group'

Сценарий: 02.02 Создание отчета с ручным вводом - 'VA - Count'

	И Я создаю вид отчета с именем 'VA - Count' и родителем 'VA - Rules rascheta (group)'
	
	* Редактируем структуру отчета
		И Я открываю контруктор отчета с именем 'VA - Count'
		И Я в конструкторе отчета добавляю строку с именем 'Products'
	...
	И Я Для вида отчета 'VA - Count' я создаю бланк по умолчанию
	И Я Для вида отчета 'VA - Count' в бланке для группы раскрытия с адресом 'R8C1' задаю сортировку 'Products.Product ID'

Сценарий: 02.03 Создание экземпляра отчета - 'VA - Count'

	И Я создаю экземпляр отчета для вида отчета 'VA - Count' сценарий 'VA - Main scenario' период '1/1/2021' '3/31/2021' периодичность "Month" организация 'Mercury LLC' проект '' аналитики '' '' '' '' '' '' 
	И я запоминаю текущее окно как 'WindowTitle'

	И Я добавляю значения с раскрытием показателей в ячейку 'R6C2'
		| 'VA0Product'                                                     | 'Value' |
		| '1C:Enterprise 8.3 CORP. Server License (x86-64)'           | '20.00000' |
		| '1C:Enterprise 8 CORP. Client license for 100 users' | '40.00000' |
		| '1C:Corporate performance management'                                      | '5.00000'  |
		| '1C:Corporation'                                                  | '7.00000'  |
	...
	* Проверим результат
		...
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| 'Products'                                                    | '80'             | '130'             | '160'          | '370'        |
			| '1C:Corporate performance management '                                      | '5'              | '15'              | '10'           | '30'         |
			| '1C:Corporation '                                                  | '7'              | '17'              | '14'           | '38'         |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '20'             | '30'              | '40'           | '90'         |
			| '1C:ERP. Corporate performance management '                                   | '8'              | '18'              | '16'           | '42'         |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '40'             | '50'              | '80'           | '170'        |
...
Сценарий: 02.04 Создание отчета с загрузкой из РС 'VA - Price'
	...
	И Я создаю вид отчета с именем 'VA - Price' и родителем 'VA - Rules rascheta (group)'
	...
	* Вводим формулы расчета
	...
	И Я Для вида отчета 'VA - Price' в бланке для группы раскрытия с адресом 'R8C1' задаю сортировку 'Products.Product ID'
...
Сценарий: 02.05 Создание экземпляра отчета - 'VA - Price'
	...
	* Проверим результат
		...
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| 'Products'                                                    | '6,030,000'      | '6,633,000'       | '7,495,400'    | '20,158,400' |
			| '1C:Corporate performance management '                                      | '1,250,000'      | '1,375,000'       | '1,553,800'    | '4,178,800'  |
			| '1C:Corporation '                                                  | '2,050,000'      | '2,255,000'       | '2,548,200'    | '6,853,200'  |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '180,000'        | '198,000'         | '223,700'      | '601,700'    |
			| '1C:ERP. Corporate performance management '                                   | '1,950,000'      | '2,145,000'       | '2,423,900'    | '6,518,900'  |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '600,000'        | '660,000'         | '745,800'      | '2,005,800'  |

Сценарий: 02.06 Создание отчета с рассчитываемыми показателями - 'VA - Sales'

	И Я создаю вид отчета с именем 'VA - Sales' и родителем 'VA - Rules rascheta (group)'
	...
	* Вводим формулы расчета
		...
		И Я выбираю показатель с кодом 'Products_Count' вида отчета  'VA - Count'
		Тогда открылось окно "Edit tree *"
		И я нажимаю на кнопку с именем 'WriteAndCollapse'
		Тогда открылось окно "Edit tree"
		...
		И в табличном документе 'SpreadsheetFieldTemlate' я делаю двойной клик на текущей ячейке
		И я нажимаю на кнопку с именем 'RefToIndicator1'
		И Я выбираю показатель с кодом 'Products_Price' вида отчета 'VA - Price'
		Тогда открылось окно "Edit tree *"
		И я нажимаю на кнопку с именем 'WriteAndCollapse'
		Тогда открылось окно "Edit tree"
		...
		И в табличном документе 'SpreadsheetFieldTemlate' я делаю двойной клик на текущей ячейке
		И я нажимаю на кнопку с именем 'RefToIndicator1'
		И Я выбираю показатель с кодом 'Products_Count'
		Тогда открылось окно "Edit tree *"
		И я нажимаю на кнопку с именем 'ButtonMultiply'
		И я нажимаю на кнопку с именем 'RefToIndicator1'
		И Я выбираю показатель с кодом 'Products_Price'
		Тогда открылось окно "Edit tree *"
		И я нажимаю на кнопку с именем 'WriteAndCollapse'	
	...
	И Я Для вида отчета 'VA - Sales' в бланке для группы раскрытия с адресом 'R8C1' задаю сортировку 'Products.Product ID'
...
Сценарий: 02.07 Создание экземпляра отчета - 'VA - Sales'	- пишем черновые версии
	...
	* Расчитаем документ
		...
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| ''                                                                | 'January 2021' | ''          | ''           | 'February 2021' | ''          | ''            | 'March 2021' | ''          | ''            | 'TOTAL'      | ''           | ''            |
			| ''                                                                | 'Count'     | 'Price'      | 'Sum'      | 'Count'      | 'Price'      | 'Sum'       | 'Count'   | 'Price'      | 'Sum'       | 'Count' | 'Price'       | 'Sum'       |
			| 'Products'                                                    | '80'             | '6,030,000' | '63,800,000' | '130'             | '6,633,000' | '136,510,000' | '160'          | '7,495,400' | '158,607,200' | '370'        | '20,158,400' | '358,917,200' |
			| '1C:Corporate performance management '                                      | '5'              | '1,250,000' | '6,250,000'  | '15'              | '1,375,000' | '20,625,000'  | '10'           | '1,553,800' | '15,538,000'  | '30'         | '4,178,800'  | '42,413,000'  |
			| '1C:Corporation '                                                  | '7'              | '2,050,000' | '14,350,000' | '17'              | '2,255,000' | '38,335,000'  | '14'           | '2,548,200' | '35,674,800'  | '38'         | '6,853,200'  | '88,359,800'  |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '20'             | '180,000'   | '3,600,000'  | '30'              | '198,000'   | '5,940,000'   | '40'           | '223,700'   | '8,948,000'   | '90'         | '601,700'    | '18,488,000'  |
			...
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '40'             | '600,000'   | '24,000,000' | '50'              | '660,000'   | '33,000,000'  | '80'           | '745,800'   | '59,664,000'  | '170'        | '2,005,800'  | '116,664,000' |
...
Сценарий: 02.09 Проверка поведения формы редактирования процедур
	...
	* Откроем процедуру ДО вычисления
		...
		Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
			|''|
			|''|
	...
	* Ввод показателей
		...
		И Я выбираю показатель с кодом 'Products_Count'
		Когда открылось окно 'VA - Sales, VA - Sales: CalculationProcedure *'
		Если '$$LanguageИнтерфейса$$ = "Ru"' Тогда
			Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
				|' IndicatorsArray = New Array;'|
				|'IndicatorsArray.Add(\"VASales:Products_Count\");'|
				|'tIndicatorsValues = GetIndicatorsValue(New Structure(\"ReportsIndicators\", IndicatorsArray));'|
				|''|							
		Иначе
			Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
				|' ArrayIndicators = New Array;'|
				|'ArrayIndicators.Add(\"VASales:Products_Count\");'|
		...
		Если '$$LanguageИнтерфейса$$ = "Ru"' Тогда
			Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
				|' IndicatorsArray = New Array;'|
				|'IndicatorsArray.Add(\"VAPrice:Products_Price\");'|
				|'tIndicatorsValues = GetIndicatorsValue(New Structure(\"ReportsIndicators\", IndicatorsArray));'|
				|''|						
		Иначе
			Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
				|' ArrayIndicators = New Array;'|
				|'ArrayIndicators.Add(\"VAPrice:Products_Price\");'|
				|'tValuesIndicators = UGetIndicatorsValue(New Structure(\"IndicatorsReports\", ArrayIndicators));'|
				|''|
		И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '	
	...
	* Ввод функций
		Если '$$LanguageИнтерфейса$$ = "Ru"' Тогда
			...
			* УстановитьЗначениеПоказателя
				...
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
					|' SetIndicatorValue('|
					|'	<код pokazatelja или ссылка to1 показатель>,'|
					|'	<значение pokazatelja>,'|
					|'	[<период>],'|
					|'	[<аналитика 1>],'|
					|'	[<аналитика 2>],'|
					|'	[<аналитика 3>],'|
			...
			* ДатаПринадлежитПериоду
				И в таблице 'Definitions' я перехожу к строке:
					...
					| 'DateBelongsToPeriod()' |
				И в таблице 'Definitions' я выбираю текущую строку
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен ' DateBelongsToPeriod(<дата>, <период>);'
				И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '
			* ОтносительныйПериод
				И в таблице 'Definitions' я перехожу к строке:
					| "Method"                 |
				...
				И в таблице 'Definitions' я выбираю текущую строку
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
					|' RelativePeriod(<ishodniy период>, [<смещение>]);'|
					|''|
				И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '
			* ОтносительныйПериодГод
				...
				И в таблице 'Definitions' я выбираю текущую строку
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен 
					|' RelativePeriodYear(<ishodniy период>, [<смещение>]);'|
					|''|
				И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '
			* СоставПериметра
				И в таблице 'Definitions' я перехожу к строке:
			...
			* ВернутьМассивОрганизацийОсуществляющихКонтрольПериметра
				И в таблице 'Definitions' я перехожу к строке:
					...
					| 'CompaniesControllingPerimeter()' |
				И в таблице 'Definitions' я выбираю текущую строку
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен ' CompaniesControllingPerimeter();'
				И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '
			* ОчиститьВсе
				И в таблице 'Definitions' я перехожу к строке:
					| "Method"         |
		...
		Иначе
			...
			* ДатаПринадлежитПериоду
				И в таблице 'Definitions' я перехожу к строке:
					...
					| 'UDateBelongsToPeriod()' |
				И в таблице 'Definitions' я выбираю текущую строку	
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен ' UDateBelongsToPeriod(<StringDate>, <Period>);'				
				И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '
			* ОтносительныйПериод
				И в таблице 'Definitions' я перехожу к строке:
					| "Method"             |
			...
			* СоставПериметраДляЭлиминации
				И в таблице 'Definitions' я перехожу к строке:
					...
					| 'UPerimeterContentForElimination()' |
				И в таблице 'Definitions' я выбираю текущую строку
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен ' UPerimeterContentForElimination();'
				И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '
			* ВернутьМассивОрганизацийОсуществляющихКонтрольПериметра
				И в таблице 'Definitions' я перехожу к строке:
					| "Method"                           |
					| 'UPerimeterControllingEntities()' |
				И в таблице 'Definitions' я выбираю текущую строку
				Тогда элемент формы с именем 'TextDocFieldProcedure' стал равен ' UPerimeterControllingEntities();'
				И в поле с именем 'TextDocFieldProcedure' я ввожу текст ' '
			* ОчиститьВсе
				И в таблице 'Definitions' я перехожу к строке:
					| "Method"       |
	...
	* Сохраним изменения
		...
		Тогда открылось окно 'VA - Sales, VA - Sales: CalculationProcedure *'
		И я нажимаю на кнопку с именем 'FormWriteAndClose'
		Тогда открылось окно "1C:Enterprise"
		И я нажимаю на кнопку с именем 'Button0'
		Тогда открылось окно '$WindowTitle$ *'
...
Сценарий: 02.10 Проверка работы произвольных функций расчета
	...
	* Вводим текст процедуры
		...
		Когда открылось окно 'VA - Sales, VA - Sales: ProcedureAfterCalculation'
		Если '$$LanguageИнтерфейса$$ = "Ru"' Тогда
			И в поле с именем 'TextDocFieldProcedure' я ввожу текст 
				|'ReportData = New Structure("Company,Scenario,Currency,ReportPeriod",'|
				|' SettlementObject.Company,SettlementObject.Scenario,SettlementObject.MainCurrency,SettlementObject.PeriodsArray);'|
				...
				|' Catalogs.ReportsIndicators.FindByCode("Products_Price",True,,SettlementObject.ReportKind));'|
				...
				|'ClearAll();'|
				|''|
				|'For Each StringValue2 From tIndicatorsValues_Count Do'|
				|''|
				...
				|'EndOfTheCycle;'|
				|''|
				|'For Each StringValue2 From tIndicatorsValues_Price Do'|
				|''|
		...
		Иначе
			И в поле с именем 'TextDocFieldProcedure' я ввожу текст 
				...
				|''|
				|'ReportData.Insert(\"ReportIndicator\", '|
				|'  Catalogs.ReportsIndicators.FindByCode(\"Products_Price\",True,,CalcStructure.ReportKind));'|
	...
	* Сравниваем результат
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| ''                                                                | 'January 2021' | ''     | ''      | 'February 2021' | ''           | ''      | 'March 2021' | ''           | ''      | 'TOTAL'      | ''           | ''      |
			| ''                                                                | 'Count'     | 'Price' | 'Sum' | 'Count'      | 'Price'       | 'Sum' | 'Count'   | 'Price'       | 'Sum' | 'Count' | 'Price'       | 'Sum' |
			| 'Products'                                                    | '0'              | '0'    | '0'     | '130'             | '12,060,000' | '0'     | '180'          | '13,266,000' | '0'     | '310'        | '25,326,000' | '0'     |
			| '1C:Corporate performance management '                                      | '0'              | '0'    | '0'     | '15'              | '2,500,000'  | '0'     | '25'           | '2,750,000'  | '0'     | '40'         | '5,250,000'  | '0'     |
			| '1C:Corporation '                                                  | '0'              | '0'    | '0'     | '17'              | '4,100,000'  | '0'     | '27'           | '4,510,000'  | '0'     | '44'         | '8,610,000'  | '0'     |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '0'              | '0'    | '0'     | '30'              | '360,000'    | '0'     | '40'           | '396,000'    | '0'     | '70'         | '756,000'    | '0'     |
			| '1C:ERP. Corporate performance management '                                   | '0'              | '0'    | '0'     | '18'              | '3,900,000'  | '0'     | '28'           | '4,290,000'  | '0'     | '46'         | '8,190,000'  | '0'     |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '0'              | '0'    | '0'     | '50'              | '1,200,000'  | '0'     | '60'           | '1,320,000'  | '0'     | '110'        | '2,520,000'  | '0'     |
...
Сценарий: 02.11 Отображение экземпляра в валюте после расчетов
	...
	* Проверим суммы в валюте
		...
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| ''                                                                | 'January 2021' | ''     | ''      | 'February 2021' | ''        | ''      | 'March 2021' | ''        | ''      | 'TOTAL'      | ''        | ''      |
			| ''                                                                | 'Count'     | 'Price' | 'Sum' | 'Count'      | 'Price'    | 'Sum' | 'Count'   | 'Price'    | 'Sum' | 'Count' | 'Price'    | 'Sum' |
			| 'Products'                                                    | '0'              | '0'    | '0'     | '130'             | '134,101' | '0'     | '180'          | '149,591' | '0'     | '310'        | '283,692' | '0'     |
			| '1C:Corporate performance management '                                      | '0'              | '0'    | '0'     | '15'              | '27,799'  | '0'     | '25'           | '31,010'  | '0'     | '40'         | '58,808'  | '0'     |
			| '1C:Corporation '                                                  | '0'              | '0'    | '0'     | '17'              | '45,590'  | '0'     | '27'           | '50,856'  | '0'     | '44'         | '96,446'  | '0'     |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '0'              | '0'    | '0'     | '30'              | '4,003'   | '0'     | '40'           | '4,465'   | '0'     | '70'         | '8,468'   | '0'     |
			| '1C:ERP. Corporate performance management '                                   | '0'              | '0'    | '0'     | '18'              | '43,366'  | '0'     | '28'           | '48,375'  | '0'     | '46'         | '91,741'  | '0'     |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '0'              | '0'    | '0'     | '50'              | '13,343'  | '0'     | '60'           | '14,885'  | '0'     | '110'        | '28,228'  | '0'     |
