...
Сценарий: 04.01 Создание вида отчета 'VA - Arbitrary request'
	...
	* Редактируем структуру отчета
	...
	И Я Для вида отчета 'VA - Arbitrary request' в бланке для группы раскрытия с адресом 'R8C1' задаю сортировку 'Products.Product ID'
...
Сценарий: 04.02 Проверка поведения формы настройки показателей
	...
	* Проверяем заполнение таблиц
		...
		Тогда Таблица 'MappingTable' содержит '3' строк из списка:
			...
			| 'Dimension 1: Products' | "Source field"    | 'Products' | 'Products'  |
...
Сценарий: 04.03 Создание экземпляра отчета - 'VA - Main scenario' с использованием многопериодного контекста
...
Сценарий: 04.05 Создание экземпляра отчета - 'VA - Main scenario' без использования многопериодного контекста
