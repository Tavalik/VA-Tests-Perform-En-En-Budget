...
Сценарий: 00.01 Установка значений констант, определение типа приложения
	...
	И я закрываю TestClient 'CPM - CashManagement'
	И я закрываю TestClient 'CPM - Purchases'
	И я закрываю TestClient 'CPM - Дымовые'

	И Инициализация переменных

	И Я создаю единицу измерения с кодом '796' если ее нет, наименование 'PCs', наименование полное 'Штука'

	И Я устанавливаю в константу 'ShouldUseIndicatorRecalculationTraceMode' значение 'True'
	...
	* Добавим валюты, если их нет
		...
		* USD
			И Я создаю валюту с цифровым кодом '840' символьным кодом 'USD' и наименованием 'Доллар From1ШA' если ее нет
	...
	* Устанавливаем валюту регламентированного учета для ЕРПУХ
	...
	Если '$$ЭтоPerform$$' Тогда
		И Я создаю страну с кодом1 '643' кодом2 'RU' кодом3 'RUS' именем1 'Russia' именем2 'Российская Федерация' именем3 'The Russian Federation' если ее нет в 1C:CPMWE
	...
	* Отключаем работу с новостями
		Если '$$IsERPCPM$$' Тогда
			И я устанавливаю в константу 'IsWorkWithNewsAllowed' значение 'False'
			И я устанавливаю в константу 'IsWorkWithNewsAllowedЧерезInternet' значение 'False'
...
Сценарий: 00.04 Создание организаций и ЦФО
	...
	* Создаем организацию Марс
		И Я создаю организацию с именем 'Mars LLC' типом 'SelectCenterForFinancialResponsibility' видом '' налогооблажением ''
		...
		Если '$$ЭтоPerform$$' Тогда	
			И Я для организации 'Mars LLC' для реквизита 'GroupCounterparty' выбираю значение 'VA - Subsidiary'
		Иначе			
			И Я для организации 'Mars LLC' для реквизита 'GroupКонтрагентов' выбираю значение 'VA - Subsidiary'
...
Сценарий: 00.08 Создание Регламента подготовки отчетности
	...
	* Скроем организации включим периметры
		...
		И я нажимаю на кнопку с именем 'BusinessUnitsFind'
		Тогда открылась форма с именем 'UniversalListFindExtForm'
	...
	* Установим валюту, если необходимо		
		...
		Если в таблице 'GroupStructure' есть строка Тогда
			...
			| 'VA - Package CFR'  | 'Venus LLC'                                             | ''                | "CFR"                  | 'CenterForFinancialResponsibility'                | '100'                 |
			И в таблице 'GroupStructure' я перехожу к строке:
				| "Company kind" | "Consolidating company\n/ Business unit" | "Primary currency" | "Intercompany transactions exclusion method" | "Relation to group" | "Effective share, %" |
				| 'VA - Package CFR'  | 'Venus LLC'                                             | ''                | "CFR"                  | 'CenterForFinancialResponsibility'                | '100'                 |
			И в таблице 'GroupStructure' я активизирую поле с именем 'GroupStructureCurrency'
		...
		Если в таблице 'GroupStructure' есть строка Тогда
			...
			И в таблице 'GroupStructure' я перехожу к строке:
				| "Company kind" | "Consolidating company\n/ Business unit" | "Primary currency" | "Intercompany transactions exclusion method" | "Relation to group" | "Effective share, %" |
				| 'VA - Package CFR'  | 'Earth LLC'                                              | ''                | "CFR"                  | 'CenterForFinancialResponsibility'                | '100'                 |
			И в таблице 'GroupStructure' я активизирую поле с именем 'GroupStructureCurrency'
			И в таблице 'GroupStructure' я выбираю текущую строку
			И в таблице 'GroupStructure' из выпадающего списка с именем 'GroupStructureCurrency' я выбираю точное значение 'RUB'
		Если в таблице 'GroupStructure' есть строка Тогда
			| "Company kind" | "Consolidating company\n/ Business unit" | "Primary currency" | "Intercompany transactions exclusion method" | "Relation to group" | "Effective share, %" |
			| 'VA - Package CFR'  | 'Mars LLC'                                               | ''                | "CFR"                  | 'CenterForFinancialResponsibility'                | '100'                 |
			И в таблице 'GroupStructure' я перехожу к строке:
				| "Company kind" | "Consolidating company\n/ Business unit" | "Primary currency" | "Intercompany transactions exclusion method" | "Relation to group" | "Effective share, %" |
				| 'VA - Package CFR'  | 'Mars LLC'                                               | ''                | "CFR"                  | 'CenterForFinancialResponsibility'                | '100'                 |
			И в таблице 'GroupStructure' я активизирую поле с именем 'GroupStructureCurrency'
	...
	* Тестируем отчеты и связанные ссылки
		...
		И Я закрываю окно "Organizational structure briefly"
		Тогда открылось окно '$WindowTitle$'
		И я нажимаю на кнопку с именем 'FormDocumentScenarioPeriodManagementAccountingPeriodManagementDocs'
...
Сценарий: 00.10 Создание Видов аналитики (корпоративные)
	...
	Иначе
		И Я создаю вид аналитики с кодом 'VA0ProCate' именем 'Product categories' и типом 'CatalogRef.ТоварныеCategories'
	И Я создаю вид аналитики с кодом 'VA0Product' именем 'Products' и типом 'CatalogRef.Products'
	И Я создаю вид аналитики с кодом 'VA0Counter' именем 'Counterparties' и типом 'CatalogRef.Counterparties'
	И Я создаю вид аналитики с кодом 'VA0Contrac' именем 'Contracts' и типом 'CatalogRef.CounterpartiesContracts'
	И Я создаю вид аналитики с кодом 'VA0CFItems' именем 'Cash flow items' и типом 'CatalogRef.CashFlowItems'
...
Сценарий: 00.13 Создание Номенклатуры
	...
	И Я создаю номенклатуру с именем '1C:Enterprise 8.3 CORP. Server License (x86-64)' родителем 'VA - Products' видом номенклатуры 'VA - Products' артикулом '2900001916059'
	И Я создаю номенклатуру с именем '1C:Enterprise 8 CORP. Client license for 100 users' родителем 'VA - Products' видом номенклатуры 'VA - Products' артикулом '4601546106674'
	И Я создаю номенклатуру с именем '1C:Corporate performance management' родителем 'VA - Products' видом номенклатуры 'VA - Products' артикулом '2900001871389'
	И Я создаю номенклатуру с именем '1C:Corporation' родителем 'VA - Products' видом номенклатуры 'VA - Products' артикулом '2900001871419'
	...
	Если '$$IsCPM$$' Тогда
		И Я Для номенклатуры с именем '1C:Enterprise 8.3 CORP. Server License (x86-64)' для реквизита 'ProductCategory' выбираю значение 'VA - Product range group' в группе ''
		И Я Для номенклатуры с именем '1C:Enterprise 8 CORP. Client license for 100 users' для реквизита 'ProductCategory' выбираю значение 'VA - Product range group' в группе ''
		И Я Для номенклатуры с именем '1C:Corporate performance management' для реквизита 'ProductCategory' выбираю значение 'VA - Product range group' в группе ''
		И Я Для номенклатуры с именем '1C:Corporation' для реквизита 'ProductCategory' выбираю значение 'VA - Product range group' в группе ''
		И Я Для номенклатуры с именем '1C:ERP. Corporate performance management' для реквизита 'ProductCategory' выбираю значение 'VA - Product range group' в группе ''
		
		Если 'NOT $$ЭтоPerform$$' Тогда
			И Я Для номенклатуры с именем '1C:Enterprise 8.3 CORP. Server License (x86-64)' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'GroupPurchasesCPM'
			И Я Для номенклатуры с именем '1C:Enterprise 8 CORP. Client license for 100 users' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'GroupPurchasesCPM'
			И Я Для номенклатуры с именем '1C:Corporate performance management' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'GroupPurchasesCPM'
			И Я Для номенклатуры с именем '1C:Corporation' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'GroupPurchasesCPM'
	...
	ИначеЕсли '$$IsERPCPM$$' Тогда
		И Я Для номенклатуры с именем '1C:Enterprise 8.3 CORP. Server License (x86-64)' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'СворачиваемаяGroupПланированиеANDМаркетинг'
		И Я Для номенклатуры с именем '1C:Enterprise 8 CORP. Client license for 100 users' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'СворачиваемаяGroupПланированиеANDМаркетинг'
		И Я Для номенклатуры с именем '1C:Corporate performance management' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'СворачиваемаяGroupПланированиеANDМаркетинг'
		И Я Для номенклатуры с именем '1C:Corporation' для реквизита 'ТоварнаяCategory' выбираю значение 'Software products' в группе 'СворачиваемаяGroupПланированиеANDМаркетинг'
...
Сценарий: 00.14 Установка цен номенклатуры
	...
	Если '$$IsCPM$$' Тогда
		...
		Если '$$ЭтоPerform$$' Тогда
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Enterprise 8.3 CORP. Server License (x86-64)' на дату '1/1/2021' в значение '180000.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Enterprise 8 CORP. Client license for 100 users' на дату '1/1/2021' в значение '600000.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Corporate performance management' на дату '1/1/2021' в значение '1250000.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Corporation' на дату '1/1/2021' в значение '2050000.00' 'RUB' в 1C:CPMWE
			...
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Enterprise 8.3 CORP. Server License (x86-64)' на дату '2/1/2021' в значение '198000.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Enterprise 8 CORP. Client license for 100 users' на дату '2/1/2021' в значение '660000.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Corporate performance management' на дату '2/1/2021' в значение '1375000.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Corporation' на дату '2/1/2021' в значение '2255000.00' 'RUB' в 1C:CPMWE
			...
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Enterprise 8.3 CORP. Server License (x86-64)' на дату '3/1/2021' в значение '223700.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Enterprise 8 CORP. Client license for 100 users' на дату '3/1/2021' в значение '745800.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Corporate performance management' на дату '3/1/2021' в значение '1553800.00' 'RUB' в 1C:CPMWE
			И Я устанавливаю цену с типом 'VA - Products' номенклатуры '1C:Corporation' на дату '3/1/2021' в значение '2548200.00' 'RUB' в 1C:CPMWE
		...
		Иначе
			* 1С:УХ	
				...
				И из выпадающего списка с именем 'PricesType' я выбираю по строке 'VA - Products'

				// Товары
				Когда открылось окно "Install цен номенклатуры (create) *"
				И в таблице 'Goods' я нажимаю на кнопку с именем 'GoodsAdd'
				И в таблице 'Goods' из выпадающего списка с именем 'GoodsProducts' я выбираю по строке '1C:Enterprise 8.3 CORP. Server License (x86-64)'
				И в таблице 'Goods' в поле с именем 'GoodsPrice' я ввожу текст '180000.00'
				И в таблице 'Goods' я нажимаю на кнопку с именем 'GoodsAdd'
				И в таблице 'Goods' из выпадающего списка с именем 'GoodsProducts' я выбираю по строке '1C:Enterprise 8 CORP. Client license for 100 users'
				И в таблице 'Goods' в поле с именем 'GoodsPrice' я ввожу текст '600000.00'
				И в таблице 'Goods' я нажимаю на кнопку с именем 'GoodsAdd'
				И в таблице 'Goods' из выпадающего списка с именем 'GoodsProducts' я выбираю по строке '1C:Corporate performance management'
				И в таблице 'Goods' в поле с именем 'GoodsPrice' я ввожу текст '1250000.00'
				И в таблице 'Goods' я нажимаю на кнопку с именем 'GoodsAdd'
				И в таблице 'Goods' из выпадающего списка с именем 'GoodsProducts' я выбираю по строке '1C:Corporation'
				...
				Когда открылось окно "Install цен номенклатуры"
				И в таблице 'List' я выбираю текущую строку
				Тогда открылось окно "Install цен номенклатуры * , created on *"
				Тогда таблица 'Goods' стала равной:
				| 'n' | "Product range"                                                   | "Price"         | "Currency" | "Indent" |
				| '1' | '1C:Enterprise 8.3 CORP. Server License (x86-64)'           | '223,700.00'   | 'RUB'    | ''       |
				| '2' | '1C:Enterprise 8 CORP. Client license for 100 users' | '745,800.00'   | 'RUB'    | ''       |
				| '3' | '1C:Corporate performance management'                                      | '1,553,800.00' | 'RUB'    | ''       |
				| '4' | '1C:Corporation'                                                  | '2,548,200.00' | 'RUB'    | ''       |
				| '5' | '1C:ERP. Corporate performance management'                                   | '2,423,900.00' | 'RUB'    | ''       |
				И Я закрываю окно "Install цен номенклатуры * , created on *"				
	...
	Если '$$IsERPCPM$$' Тогда	
		...
		* Создаем новый вид цен
			...
			И Пока в таблице 'List' количество строк 'больше' 0 Тогда
				И Я запоминаю значение выражения '"Delete_" + StrReplace(New UUID, "-", "")' в переменную 'UID'
				И в таблице 'List' я выбираю текущую строку
				Тогда открылось окно "inA - Product range (Kind цены)"
				...
				И в поле с именем 'Description' я ввожу значение переменной 'UID'
				И я нажимаю на кнопку с именем 'FormWriteAndClose'
				И я жду закрытия окна "inA - Product range (Kind цены) *" в течение 20 секунд
			И я нажимаю на кнопку с именем 'FormCreate'
			...
			И я изменяю флаг с именем 'UseПриОптовойПродаже'
			И я изменяю флаг с именем 'UseПриРозничнойПродаже'
			И я изменяю флаг с именем 'UseПриПередачеПродукцииДавальцу'
			И я изменяю флаг с именем 'UseПриВыпускеПродукции'
		...
		* Вводим документ за Январь
			...
			И в таблице 'TreeЦен' из выпадающего списка с именем 'TreeЦенProducts' я выбираю по строке '1C:Enterprise 8.3 CORP. Server License (x86-64)'
			И в таблице 'TreeЦен' в поле с именем 'TreeЦенKindЦены*' я ввожу текст '180000.00'
			И в таблице 'TreeЦен' я нажимаю на кнопку с именем 'TreeЦенAddНоменклатуру'
			И в таблице 'TreeЦен' из выпадающего списка с именем 'TreeЦенProducts' я выбираю по строке '1C:Enterprise 8 CORP. Client license for 100 users'
			И в таблице 'TreeЦен' в поле с именем 'TreeЦенKindЦены*' я ввожу текст '600000.00'
			И в таблице 'TreeЦен' я нажимаю на кнопку с именем 'TreeЦенAddНоменклатуру'
			И в таблице 'TreeЦен' из выпадающего списка с именем 'TreeЦенProducts' я выбираю по строке '1C:Corporate performance management'
			И в таблице 'TreeЦен' в поле с именем 'TreeЦенKindЦены*' я ввожу текст '1250000.00'
			И в таблице 'TreeЦен' я нажимаю на кнопку с именем 'TreeЦенAddНоменклатуру'
			И в таблице 'TreeЦен' из выпадающего списка с именем 'TreeЦенProducts' я выбираю по строке '1C:Corporation'
		...
		* Вводим документ за Февраль
			...
			И я нажимаю на кнопку с именем 'MoveToDoc'
			Тогда открылось окно "Install цен номенклатуры (create) *"
			...
			И я нажимаю на кнопку с именем 'OK'
			Тогда открылось окно "Install цен номенклатуры (create) *"
			Если элемент формы '№ в predelah day' присутствует на форме Тогда	
				И в поле с именем 'NumberINПределахДня' я ввожу текст '1'
			И я нажимаю на кнопку с именем 'FormPostAndClose'
			И я жду закрытия окна "Install цен номенклатуры (create) *" в течение 20 секунд
		...
		* Проверяем результат
			...
			| '1C:Enterprise 8.3 CORP. Server License (x86-64)'           | '<характеристики Not используются>' | '223,700.00'   | ''       |
			| '1C:Enterprise 8 CORP. Client license for 100 users' | '<характеристики Not используются>' | '745,800.00'   | ''       |
			| '1C:Corporate performance management'                                      | '<характеристики Not используются>' | '1,553,800.00' | ''       |
			| '1C:Corporation'                                                  | '<характеристики Not используются>' | '2,548,200.00' | ''       |
...
Сценарий: 00.15 Создание Контрагентов и Договоров контрагентов
	...
	Если '$$IsERPCPM$$' Тогда
		...
		И Для партнера 'LLC \"Io\"' контрагента 'LLC \"Io\"' я создаю договор с видом "With customer" номер 'Io-001' от '1/1/2021' организация 'Mercury LLC' валюта 'RUB' сумма '10000.00' соглашение 'VA - Agreement with the client' в 1C:ERPУХ		
...
Сценарий: 00.17 Создание Статьей доходов и расходов
	...
	И Я создаю статью ДиР с именем 'Software sale' родителем 'VA - Income and expense items'
	И Я создаю статью ДиР с именем 'Software implementation' родителем 'VA - Income and expense items'

Сценарий: 00.18 Создание документа 'Отражение фактических data_'
	...
	Если 'NOT $$ЭтоPerform$$' Тогда
		...
		* Создаем новый документ №1
			...
			* Страница 'Основное'	
				...
				И в поле с именем 'Date' я ввожу текст '01.01.2021  12:00:00 AM'
				И из выпадающего списка с именем 'DBDoc' я выбираю по строке 'GoodsServicesSales'		
				И из выпадающего списка с именем 'Scenario' я выбираю по строке 'VA - Main scenario'
				И из выпадающего списка с именем 'Company' я выбираю по строке 'Mercury LLC'
				...
				И из выпадающего списка с именем 'IncomeExpense' я выбираю точное значение "Income"
			...
			* Бюджет движения денежных средств
				...
				Если '$$IsERPCPM$$' Тогда
					И в таблице 'TabCashFlowsBudget' я нажимаю кнопку выбора у реквизита с именем 'TabCashFlowsBudgetCounterpartyAgreement'
					Тогда открылось окно "Select data type"
				...
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCounterpartyAgreement' я выбираю по строке 'Ganymede-001'
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCashFlowItem' я выбираю по строке 'Software sale'
				...
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCounterpartyAgreement' я выбираю по строке 'Ganymede-002'
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCashFlowItem' я выбираю по строке 'Software implementation'
			...
			* Запишем документ, проверим движения
				...
				Тогда открылось окно "Отражение фактических данных * , created on *"
				И я нажимаю на кнопку с именем 'FormProcessingDocumentRegisterRecordsDocumentRegisterRecordsReport'
				Тогда открылось окно "Document register records: Отражение фактических данных * , created on *"
				Если '$$IsCPM$$' Тогда
					И я перехожу к закладке с именем 'GroupRegRecordsДенежныхСредствCPM'
					Тогда таблица 'CashFlowsCPM' стала равной:
						...
						| '1' | 'Software sale' | ''                        | 'Receipt'        | 'Non_Cash'          | 'LLC "Ganymede"'                      | ''           | 'Ganymede-001 dated 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '780,000.00'       | '780,000.00'   |
						| '2' | 'Software implementation'  | ''                        | 'Receipt'        | 'Non_Cash'          | 'LLC "Ganymede"'                      | ''           | 'Ganymede-002 dated 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '2,340,000.00'     | '2,340,000.00' |
				Если '$$IsERPCPM$$' Тогда
					Тогда таблица 'BudgetsActualValues' стала равной:
						| 'Document планирования' | 'n' | 'Dimension2' | 'Purpose'                   | 'CenterForFinancialResponsibility'          | 'Dimension4' | 'Item bjudzheta'                   | 'Dimension1' | 'Dimension3' | 'Dimension5' | 'Dimension6' | 'Project'               | "Document регистратор" | 'Currency' | "Income expense" | 'Sum'        | "Amount упр"    | 'Count' |
						| ''                      | '1' | ''           | 'Бюджет движения денежных sredstv' | 'Mercury LLC' | ''           | 'Software sale' | ''           | ''           | ''           | ''           | 'VA - Main project' | ''                     | 'RUB'    | ''              | '780,000.00'   | '780,000.00'   | ''           |
						| ''                      | '2' | ''           | 'Бюджет движения денежных sredstv' | 'Mercury LLC' | ''           | 'Software implementation'  | ''           | ''           | ''           | ''           | 'VA - Main project' | ''                     | 'RUB'    | ''              | '2,340,000.00' | '2,340,000.00' | ''           |

			* Закроем документ	
				Когда открылось окно "Document register records: Отражение фактических данных * , created on *"
				И Я закрываю окно "Document register records: Отражение фактических данных * , created on *"
				Тогда открылось окно "Отражение фактических данных * , created on *"
				И я нажимаю на кнопку с именем 'FormPostAndClose'
				И я жду закрытия окна "Отражение фактических данных * , created on *" в течение 20 секунд
		...
		* Создаем новый документ №2
			...
			* Бюджет движения денежных средств
				...
				Если '$$IsERPCPM$$' Тогда
					...
					Тогда открылось окно "Contracts From контрагентами"
					И Я закрываю окно "Contracts From контрагентами"
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCounterpartyAgreement' я выбираю по строке 'Io-002'
				...
				Если '$$IsERPCPM$$' Тогда
					...
					Тогда открылось окно "Contracts From контрагентами"
					И Я закрываю окно "Contracts From контрагентами"
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCounterpartyAgreement' я выбираю по строке 'Io-001'
				И в таблице 'TabCashFlowsBudget' я активизирую поле с именем 'TabCashFlowsBudgetSum'
				...
				Если '$$IsERPCPM$$' Тогда
					...
					И Я закрываю окно "Cash flow items"
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCounterpartyAgreement' я выбираю по строке 'Io-001'
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCashFlowItem' я выбираю по строке 'Software upgrade'
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCenterForFinancialResponsibility' я выбираю по строке 'Mercury LLC'
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetProject' я выбираю по строке 'VA - основной проект'
