...
Сценарий: 00.04 Создание организаций и ЦФО
	...
	* Создаем организацию Система
		И Я создаю организацию с именем "System LLC" типом 'SelectРФ' видом 'SelectЮL' налогооблажением 'СистемаНалогообложенияShared3ЮL'
		Если '$$IsCPM$$' Тогда
	...
	* Создаем организацию Меркурий
		И Я создаю организацию с именем "Mercury LLC" типом 'SelectРФ' видом 'SelectЮL' налогооблажением 'СистемаНалогообложенияShared3ЮL'
...
Сценарий: 00.10 Создание Видов аналитики (корпоративные)
	...
	И Я создаю вид аналитики с кодом "VA0Contrac" именем "Counterparty contracts" и типом 'CatalogRef.CounterpartiesContracts'
...
Сценарий: 00.13 Создание Номенклатуры
	...
	Если '$$IsCPM$$' Тогда
		И Я Для номенклатуры с именем "4C:Enterprise 8.3 CORP. Server License (x86-64)" для реквизита 'ProductCategory' выбираю значение "VA - Product range group" в группе ''
		И Я Для номенклатуры с именем "3C:Enterprise 8 CORP. Client license for 100 users" для реквизита 'ProductCategory' выбираю значение "VA - Product range group" в группе ''
		И Я Для номенклатуры с именем "5C:Corporate performance management" для реквизита 'ProductCategory' выбираю значение "VA - Product range group" в группе ''
		И Я Для номенклатуры с именем "2C:Corporation" для реквизита 'ProductCategory' выбираю значение "VA - Product range group" в группе ''
...
Сценарий: 00.14 Установка цен номенклатуры
	...
	Если '$$IsCPM$$' Тогда
		...
		Иначе
			* 1С:УХ	
				...
				И в таблице 'InventoryItems' я нажимаю на кнопку с именем 'InventoryItemsAdd'
				И в таблице 'InventoryItems' из выпадающего списка с именем 'InventoryItemsProducts' я выбираю по строке "4C:Enterprise 8.3 CORP. Server License (x86-64)"
				И в таблице 'InventoryItems' в поле с именем 'InventoryItemsPrice' я ввожу текст '180000.00'
				И в таблице 'InventoryItems' я нажимаю на кнопку с именем 'InventoryItemsAdd'
				И в таблице 'InventoryItems' из выпадающего списка с именем 'InventoryItemsProducts' я выбираю по строке "3C:Enterprise 8 CORP. Client license for 100 users"
				И в таблице 'InventoryItems' в поле с именем 'InventoryItemsPrice' я ввожу текст '600000.00'
				И в таблице 'InventoryItems' я нажимаю на кнопку с именем 'InventoryItemsAdd'
				И в таблице 'InventoryItems' из выпадающего списка с именем 'InventoryItemsProducts' я выбираю по строке "5C:Corporate performance management"
				И в таблице 'InventoryItems' в поле с именем 'InventoryItemsPrice' я ввожу текст '1250000.00'
				И в таблице 'InventoryItems' я нажимаю на кнопку с именем 'InventoryItemsAdd'
				И в таблице 'InventoryItems' из выпадающего списка с именем 'InventoryItemsProducts' я выбираю по строке "2C:Corporation"
				И в таблице 'InventoryItems' в поле с именем 'InventoryItemsPrice' я ввожу текст '2050000.00'
				И в таблице 'InventoryItems' я нажимаю на кнопку с именем 'InventoryItemsAdd'
				И в таблице 'InventoryItems' из выпадающего списка с именем 'InventoryItemsProducts' я выбираю по строке "1C:ERP. Corporate performance management"
				И в таблице 'InventoryItems' в поле с именем 'InventoryItemsPrice' я ввожу текст '1950000.00'
				...
				И из выпадающего списка с именем 'PricesType' я выбираю по строке "VA - Products"
				И в таблице 'InventoryItems' я нажимаю на кнопку с именем 'FillByGroupНоменклатуры'
				...
				И в таблице 'List' я выбираю текущую строку
				Тогда открылось окно "Install цен номенклатуры (create) *"
				И в таблице 'InventoryItems' я нажимаю на кнопку с именем 'InventoryItemsChangeInventoryItems'
				Тогда открылось окно "Modify таблицы товаров"
				И из выпадающего списка с именем 'ThisAction' я выбираю точное значение "Edit цены for процент"
				...
				И я нажимаю на кнопку с именем 'Execute'
				И из выпадающего списка с именем 'ThisAction' я выбираю точное значение "Round off цены до"
				...
				Тогда таблица 'InventoryItems' стала равной:
	...
	Если '$$IsERPCPM$$' Тогда	
		...
		* Создаем новый вид цен
			...
			И я изменяю флаг с именем 'ShouldUseПриОптовойПродаже'
			И я изменяю флаг с именем 'ShouldUseПриРозничнойПродаже'
			И я изменяю флаг с именем 'ShouldUseПриПередачеПродукцииДавальцу'
			И я изменяю флаг с именем 'ShouldUseПриВыпускеПродукции'
		...
		* Вводим документ за Февраль
			...
			И в таблице 'TreeЦен' я нажимаю на кнопку с именем 'TreeЦенAddInventoryItemsOnОтбору'
			...
			И в таблице 'SettingsComposerSettingsFilter' я перехожу к строке:
				...
				| 'IN List'      | '<Пустое значение>' | "No"           | 'Products.Kind номенклатуры' | 'Ordinary'    | 'Быстрый доступ'    |
			...
			И я нажимаю на кнопку с именем 'OK'
			Тогда открылось окно "Install цен номенклатуры (create) *"
			Если элемент формы '№ в пределах day' присутствует на форме Тогда	
				И в поле с именем 'NumberINПределахДня' я ввожу текст '1'
			И я нажимаю на кнопку с именем 'FormPostAndClose'
			И я жду закрытия окна "Install цен номенклатуры (create) *" в течение 20 секунд
...
Сценарий: 00.15 Создание Контрагентов и Договоров контрагентов
	...
	Если '$$IsCPM$$' Тогда
		Если 'NOT $$ЭтоPerform$$' Тогда
			...
			И я перезаполняю константу 'ShouldUseНумерациюДоговоровСПокупателями' значением 'FALSE'			
	...
	Если 'NOT $$ЭтоPerform$$' Тогда
		...
		* Создаем новый документ №1
			...
			* Страница 'Основное'	
				...
				И в поле с именем 'Date' я ввожу текст '01.01.2021  12:00:00 AM'
				И из выпадающего списка с именем 'DBDocument' я выбираю по строке 'GoodsServicesSales'		
				И из выпадающего списка с именем 'Scenario' я выбираю по строке "VA - Main scenario"
			...
			* Запишем документ, проверим движения
				...
				Если '$$IsCPM$$' Тогда
					...
					Тогда таблица 'CashFlowsCPM' стала равной:
						...
						| '1' | "3Software sale" | ''                        | 'Приход'        | 'Non_Cash'          | 'LLC "Ганимед"'                      | ''           | 'Ганимед-001 dated 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '780,000.00'       | '780,000.00'   |
						| '2' | "2Software implementation"  | ''                        | 'Приход'        | 'Non_Cash'          | 'LLC "Ганимед"'                      | ''           | 'Ганимед-002 dated 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '2,340,000.00'     | '2,340,000.00' |
				Если '$$IsERPCPM$$' Тогда
					Тогда таблица 'BudgetsActualValues' стала равной:
						| 'Document планирования' | 'N' | 'Dimension2' | 'IntendedPurpose'                   | 'CenterForFinancialResponsibility'          | 'Dimension4' | 'Item бюджета'                   | 'Dimension1' | 'Dimension3' | 'Dimension5' | 'Dimension6' | 'Project'               | "Document регистратор" | 'Currency' | "Income expense" | 'Sum'        | "Amount упр"    | 'Count' |
		...
		* Создаем новый документ №2
			...
			* Бюджет движения денежных средств
				...
				Если '$$IsERPCPM$$' Тогда
					...
					Тогда открылось окно "Contracts From контрагентами"
					И Я закрываю окно "Contracts From контрагентами"
				И в таблице 'TabCashFlowsBudget' из выпадающего списка с именем 'TabCashFlowsBudgetCounterpartyAgreement' я выбираю по строке 'Ио-002'
