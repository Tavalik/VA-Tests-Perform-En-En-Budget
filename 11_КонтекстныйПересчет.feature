...
Сценарий: 11.04 Выгрузка и загрузка из Excel
	...
	* Грузим эталонный файл
		...
		И в поле с именем 'ExistingFile' я ввожу текст "$КаталогПроекта$\\Макеты\\ВА_КонтекстныйПересчетЭталон_En.xlsx"
	...
	* Грузим подготовленный файл
		...
		И я нажимаю на кнопку с именем 'ImportFromMSExcel'
		Тогда открылось окно "Import from Excel"
		И в поле с именем 'ExistingFile' я ввожу текст "$КаталогПроекта$\\Макеты\\ВА_КонтекстныйПересчетПодготовленный_En.xlsx"
		И я жду, что поле с именем 'SheetName' перестанет быть пустым в течение 30 секунд
		Тогда элемент формы с именем 'SheetName' стал равен 'TDSheet'
		И я нажимаю на кнопку с именем 'Load'
		И я жду открытия формы '$WindowTitle$' в течение 30 секунд	
...
Сценарий: 11.05 Тестирование формы ввода показателей
	...
	* Открытие формы редактирование, настройки отображения
		...
		И Таблица 'IndicatorsDrilldown' содержит '15' строк из списка:
			| 'ReportPeriod'    | "Product range"                                                   | 'Value1' | 'Value3' | 'Value2' | 'Value4'  | 'Value5' |
	...
	* Режим таблицы по периодам
		...
		И Таблица 'IndicatorsDrilldown' содержит '5' строк из списка:
			| "Product range"                                                   | 'Period_3_Value3' | 'Period_1_Value1' | 'Period_3_Value5' | 'Period_1_Value2' | 'Period_2_Value1' | 'Period_2_Value5' | 'Period_1_Value3' | 'Period_1_Value4' | 'Period_1_Value5' | 'Period_3_Value2' | 'Period_2_Value2' | 'Period_2_Value3' | 'Period_2_Value4' | 'Period_3_Value1' | 'Period_3_Value4' |
