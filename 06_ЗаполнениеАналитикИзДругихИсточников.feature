...
Сценарий: 06.01 Создание группы отчетов 'VA - Cumulative total'
	...
	* Вводим значения показателей
		...
		* Товары
			И Я добавляю значения с раскрытием показателей в ячейку 'R9C2'
				| 'VA0Product'                                                     | 'Value' |
				| '1C:Enterprise 8.3 CORP. Server License (x86-64)'           | '1.00000'  |
				| '1C:Enterprise 8 CORP. Client license for 100 users' | '3.00000'  |
				| '1C:Corporate performance management'                                      | '2.00000'  |
		...
		* Другой источник
			...
			* Источник с прозвольным запросом №1
				...
				И в таблице 'List' я перехожу к строке:
					...
					| 'VA - Data to add analytics_Goods count' | "Current infobase report indicator (easy setup)" | "Current infobase" | 'Goods count' |
				И в таблице 'List' я выбираю текущую строку				
				Тогда открылось окно "Data source (create)*"
				И я нажимаю на кнопку с именем 'FormWriteAndClose'
				И я жду закрытия окна "Data source (create)*" в течение 20 секунд
		...
		* Пересчет в коде
			* Основной источник
				...
				* Источник с доп. аналитиками - Статьи ДДС 
				...
				И в таблице 'MappingTable' я перехожу к строке:
					...
					| 'Dimension 3: Products' | 'Products'  | "Product range" | "Source field"    |
				И в таблице 'MappingTable' из выпадающего списка с именем 'MappingTableFillingMethod' я выбираю точное значение "Another source field"
				И в таблице 'MappingTable' я завершаю редактирование строки
				И в таблице 'MappingTable' я активизирую поле с именем 'MapTableDBAlias'
				И в таблице 'MappingTable' я выбираю текущую строку
				...
				* Устанавливаем отборы
					...
					И в таблице 'TreeOfFilterParametersDB' из выпадающего списка с именем 'DefiningMethodClarification' я выбираю по строке '1C:Corporate performance management'
			...
			* Вводим формулу расчета
				...
				Если '$$LanguageИнтерфейса$$ = "Ru"' Тогда
					И в поле с именем 'TextDocFieldProcedure' я ввожу текст 'Result = Max([BA_ЦеныНомен_Price_], 1500000)'
				Иначе
					И в поле с именем 'TextDocFieldProcedure' я ввожу текст 'Result = Max([InformationRegister_ProRanPri_Price_], 1500000)' 
				И я нажимаю на кнопку с именем 'WriteAndCollapse'
	
	И Я Для вида отчета 'VA - Analysts from fields of other sources' я создаю бланк по умолчанию
	И Я Для вида отчета 'VA - Analysts from fields of other sources' в бланке для группы раскрытия с адресом 'R8C1' задаю сортировку 'Products.Product ID'
	И Я Для вида отчета 'VA - Analysts from fields of other sources' в бланке для группы раскрытия с адресом 'R10C1' задаю сортировку 'Products.Product ID'
	И Я Для вида отчета 'VA - Analysts from fields of other sources' в бланке для группы раскрытия с адресом 'R13C1' меняю сортировку 'Products.Description' на сортировку 'Products.Product ID'
	И Я Для вида отчета 'VA - Analysts from fields of other sources' в бланке для группы раскрытия с адресом 'R16C1' меняю сортировку 'Products.Description' на сортировку 'Products.Product ID'
	И Я Для вида отчета 'VA - Analysts from fields of other sources' в бланке для группы раскрытия с адресом 'R19C1' меняю сортировку 'Products.Description' на сортировку 'Products.Product ID'
...
Сценарий: 06.05 Создание экзмемпляра отчета 'VA - Analysts from fields of other sources'
	...
	* Сравниваем результат
		...
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен:
			...
			| '1C:Corporate performance management '                                      | '4'              | '8'               | '12'           | '24'         |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '2'              | '4'               | '6'            | '12'         |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '6'              | '12'              | '18'           | '36'         |
			| 'Other source'                                                 | '15'             | '15'              | '15'           | '45'         |
			| '1C:Corporate performance management '                                      | '5'              | '5'               | '5'            | '15'         |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '5'              | '5'               | '5'            | '15'         |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '5'              | '5'               | '5'            | '15'         |
			| 'Analyst transformation'                                          | '6'              | '12'              | '18'           | '36'         |
			| 'Software upgrade '                                  | '6'              | '12'              | '18'           | '36'         |
			| '1C:Corporate performance management '                                      | '2'              | '4'               | '6'            | '12'         |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '1'              | '2'               | '3'            | '6'          |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '3'              | '6'               | '9'            | '18'         |
			| 'Calculated Source'                                         | '6'              | '12'              | '9'            | '27'         |
			| ' '                                                               | '0'              | '0'               | '9'            | '9'          |
			| '1C:Corporate performance management '                                      | '0'              | '0'               | '3'            | '3'          |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '0'              | '0'               | '3'            | '3'          |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '0'              | '0'               | '3'            | '3'          |
			| 'Software implementation '                                | '3'              | '6'               | '0'            | '9'          |
			| '1C:Corporate performance management '                                      | '1'              | '2'               | '0'            | '3'          |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '1'              | '2'               | '0'            | '3'          |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '1'              | '2'               | '0'            | '3'          |
			...
			| '1C:Corporate performance management '                                      | '1,500,000'      | '1,500,000'       | '1,553,800'    | '4,553,800'  |
			| '1C:Enterprise 8.3 CORP. Server License (x86-64) '           | '1,500,000'      | '1,500,000'       | '1,553,800'    | '4,553,800'  |
			| '1C:Enterprise 8 CORP. Client license for 100 users ' | '1,500,000'      | '1,500,000'       | '1,553,800'    | '4,553,800'  |
